...
#define THROW1    /skill/runeknight/throw1
	...
	Click()
		...
		if(!(locate(type) in usr.skills))
			...
			if(alert(usr,"Are you sure you want to buy [src]?","[src]","Yes","No")=="Yes")
				...
				var/obj/path = text2path("/obj/Skillcards/Skills/[src.name]") // this will be to store the object's path in text form
				var/tech=text2path("[path]")
				var/obj/Skill =new tech
				Skill.Move(usr)
				usr << "You learned [src]!"
		else
...
//		netherbolt {icon_state="netherbolt";reqskill=BOLT;reqskilllvl=3}
	...
	runeknight
		...
		throw1      {icon_state="throw";reqskill=STRK}
		...
		firethrow  {icon_state="firethrow";reqskill=THROW1}
